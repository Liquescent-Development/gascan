...
impl CreateRequest {
    ...
    /// Produces an invalid request for downstream fail-closed adapter tests.
    ///
    /// This is available only when the explicitly opt-in `test-support` feature
    /// is enabled and is intentionally a closed mutation set rather than an
    /// unchecked constructor.
    #[cfg(feature = "test-support")]
    #[doc(hidden)]
    pub fn test_mutated(&self, mutation: CreateRequestTestMutation) -> Self {
        let mut request = self.clone();
        match mutation {
            CreateRequestTestMutation::OwnershipManagedByMismatch => {
                request.ownership.managed_by = "foreign".to_owned();
            }
            CreateRequestTestMutation::OwnershipSandboxMismatch => {
                request.ownership.sandbox_id = SandboxId::test("mismatch");
            }
            CreateRequestTestMutation::MissingBind => request.bind_mounts.clear(),
            CreateRequestTestMutation::ExtraBind => {
                if let Some(mount) = request.bind_mounts.first().cloned() {
                    request.bind_mounts.push(mount);
                }
            }
            CreateRequestTestMutation::NoncanonicalBind => {
                if let Some(mount) = request.bind_mounts.first_mut() {
                    mount.source.push("..");
                }
            }
            ...
            CreateRequestTestMutation::ExtraVolume => {
                if let Some(mut volume) = request.volumes.first().cloned() {
                    volume.name.push_str("-extra");
                    request.volumes.push(volume);
                }
            }
            CreateRequestTestMutation::UnownedVolume => {
                if let Some(volume) = request.volumes.first_mut() {
                    volume.ownership.managed_by = "foreign".to_owned();
                }
            }
            CreateRequestTestMutation::WrongVolumeTarget => {
                if let Some(volume) = request.volumes.first_mut() {
                    volume.target = Utf8PathBuf::from("/unexpected");
                }
            }
            ...
            CreateRequestTestMutation::NonLoopbackPort => {
                if let Some(port) = request.ports.first_mut() {
                    port.host_address = IpAddr::V4(std::net::Ipv4Addr::UNSPECIFIED);
                }
            }
            CreateRequestTestMutation::DiskSome => request.resources.disk_bytes = Some(1),
            CreateRequestTestMutation::ProcessSome => request.resources.process_count = Some(1),
            CreateRequestTestMutation::MissingCpu => request.resources.cpus = None,
            CreateRequestTestMutation::MissingMemory => request.resources.memory_bytes = None,
            CreateRequestTestMutation::RootUser => request.user = RuntimeUser::Root,
            CreateRequestTestMutation::InitFalse => request.init = false,
        }
        request
    }
}
...
pub enum CreateRequestTestMutation {
    OwnershipManagedByMismatch,
    OwnershipSandboxMismatch,
    MissingBind,
    ExtraBind,
    NoncanonicalBind,
    NonwritableBind,
