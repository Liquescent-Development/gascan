...
The final post-cleanup-fix controller rerun of `network::offline_workspace_cannot_reach_external_or_host_networks` passed 1/1 in 28.41 seconds with serialized ignored-test execution. The default-network container reached the owned DNS/PF host service, proving the ordinary VM-to-host path. Public `http://example.com` and direct `http://1.1.1.1` were both unreachable from that control and were recorded as non-discriminating diagnostics. The controller environment blocks DNS queries to every resolver except its provided DNS IP, `10.10.10.53`; this environmental policy explains the public DNS diagnostic context but is not a Gascan requirement and is not hardcoded by the product or test. The offline container denied all four target roles—including the exact owned host endpoint proven reachable—with structured `configuration.networks=[]` both before and after guest-root route/interface mutation.

The captured mutation evidence was exact: `ip link add gascan0 type dummy` returned `RTNETLINK answers: Operation not permitted` with exit 2; `ip route add default via 192.0.2.1` returned the same diagnostic with exit 2. After both attempts, `ip link show` contained only the `lo` interface in `UP` state, and `ip route show` was empty. The subsequent structured inspect still reported `configuration.networks=[]`, and every offline target remained unreachable.
...
The final full live run passed 9/9 in 50.31 seconds. After the later focused cleanup-logic rerun passed 1/1 in 28.41 seconds, `sudo -n container system dns list --format json` returned exactly `[]`; no test-owned DNS mapping remained.
