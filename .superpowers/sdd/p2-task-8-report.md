...
- Real-process fake E2E: 18 tests covering live/disconnect-independent lifecycle, typed pre-begin rejection, post-begin failure, bounded unbounded-producer stdin, binary streams/logs, exact follow terminal/cancellation behavior, environment defense, two sandboxes, idle and SIGKILL restart, API mismatch, atomic token misuse, and real PTY success/nonzero/connection loss/resize/SIGINT/SIGTERM/restoration. Autostart has two separate deterministic readiness/race tests, and a real-PTY unit test covers panic unwind.
...
- Concurrent autostart serializes schema discovery/creation under SQLite's immediate transaction, then uses a deterministic client barrier and bounded connect/HTTP2/Handshake readiness probes within the fixed overall deadline; only transient startup transport failures retry, while compatibility and security failures remain terminal. Fast disconnected liveness probes whose peer credentials are no longer readable are rejected per connection instead of terminating Tonic's incoming stream. The E2E harness records the PID only after socket ownership and terminates that exact live daemon before deleting its private runtime, preventing orphan accumulation.
...
- Final exact Gate 3 passed: workspace all-target/all-feature strict Clippy, full workspace tests and doctests, formatting, and diff checks. The fake E2E suite passed 18/18, autostart 2/2, daemon 6/6, and API compatibility 10/10. Nine Apple live tests remained ignored and no Apple integration was added.

Task 8 and the broad Plan 2 branch review are clean at `2574e3c`.
