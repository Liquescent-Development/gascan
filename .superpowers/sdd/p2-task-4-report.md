...
## Exact SQLite Object Inventory Follow-up

- RED: `cargo test -p gascand --test store version_one_rejects_every_unexpected_user_schema_object -- --nocapture` failed because an extra `AFTER UPDATE ON sandboxes` trigger was accepted.
- GREEN: the focused inventory regression passed, followed by all 20 store tests.
- Schema-v1 opening now enumerates the complete main `sqlite_master` catalog and requires exactly the migration-defined four user tables, one explicit pending-operation index, and two append-only triggers.
- The only permitted SQLite-owned objects are the exact null-SQL `sqlite_autoindex_sandboxes_1` and `_2` indexes tied to `sandboxes`, plus the exact internal `sqlite_sequence` table generated by the two AUTOINCREMENT keys.
- Extra triggers on sandboxes or operations, extra indexes on required tables, extra tables/views, and unrelated tables with indexes all fail immediately with `SchemaMismatch`.
- Names beginning `sqlite_` are not blanket ignored: only the three documented exact internal objects above pass. SQLite itself reserves `sqlite_` names for internal use, so a forged user object is normally rejected at creation; if present through corruption/tooling, inventory validation rejects it.
- Inventory comparison establishes presence and absence; the existing exact structural/definition checks continue validating the contents of every allowed object.
